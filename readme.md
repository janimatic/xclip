...
- [ ] xclip exporter
- [ ] a new Multiplane fuse and/or Openfx node, to match the 2d camera camera and make precomp easier
...
- [ ] Add complementary tools to help scenes translations:
	...
	- [ ]  recursive scene parsers for each application (so that by default xclip could transfert every parameter from every node with minimal code) and limit io with name filters
- [ ] Create nodes that make translation easier :
	- [ ]  fusion multiplane camera node / fuse
    - [ ] Openfx multiplane camera node
...
(which copies luaxclip\luaxclip.dll and fusion\xclipioFusion.lua in %ProgramData%\Blackmagic Design\DaVinci Resolve\Fusion\Modules\Lua\ and copies exportXClip.lua and importXClip.lua in%AppData%\Blackmagic Design\DaVinci Resolve\Support\Fusion\Scripts\Utility)
...
- XClip C++ library example application
    ...
    The xclip test application located in /xclip/bin/Release let you test the library in c++ by editing the main.cpp file.
    It uses an example header simulating Tahoma2D keyframes data types.
...
- XClip python
    ...
    _pyxclip.pyd is the python binary library.
    This file is copied in /xclip/python/pyxclip by a cmake command, next to the pyxclip.py file generated by SWIG.
    You can test a sample python script /xclip/python/xclipTool.py
    
- XClip lua

    luaxclip.dll (or .so) is the lua binary library.
    The lua binary library is copied in /xclip/lua/luaxclip by a cmake command.
    To test the luaxclip wrapper outside of fusion, the lua 5.1 interpreter should be installed
